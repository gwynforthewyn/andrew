...
var (
	parserInstance *partialParser
	parserOnce     sync.Once
)

// getPartialParser returns the singleton partialParser instance.
func getPartialParser() *partialParser {
	parserOnce.Do(func() {
		parserInstance = &partialParser{
...
func (s Server) NewPage(pageUrl string) (Page, error) {
	...
	renderedPageContent, err := renderPartials(s.SiteFiles, pagePath, pageContent)
	if err != nil {
		return Page{}, err
	}
...
//	renderPartials parses the syntax {{ .AndrewPartialFile foo=bar bam=bas }}, finds the partial
//
...
//
// retval
// []byte: an array of bytes representing the new version of pageContent, with the partials rendered.
// error: as normal.
func renderPartials(siteFiles fs.FS, pagePath string, pageContent []byte) ([]byte, error) {
	// The parser will parse pageContent for the partial statements.
	partialParser := getPartialParser()
	...
	if matches == nil {
		slog.Debug("renderPartials", "AndrewPartialDirectiveFound", "false", "pageContent", pageContent)
	...
	for _, m := range matches {
		...
		// It's pretty common to put ' and " in the value of the k/v pair in the partial.
		...
		// can read in the partial to insert it into the web page in place of the {{ }} statement.
		partialFile, err := findPartialFile(siteFiles, pagePath, partialToFind)

		if err != nil {
			slog.Debug("renderPartials partialFile not found", "error", err)
		...
		// The tag format {{ .AndrewPartial spaceman=david }} requires parsing out the key/value pairs.
		...
		slog.Debug("renderPartials", "partialContent", partialContent)
